...
/// 
/// ### Returns
/// If `pos` begins a valid looking number literal, `identify_number()`
/// returns the character position after the end of the number.  
/// Otherwise, `identify_number()` just returns the `pos` argument.
...
mod tests {
    ...
    #[test]
    fn identify_number_will_not_panic() {
        println!("{}", 0x1E+9);
        // Near the end of `raw`.
        assert_eq!(identify("", 0), 0); // empty string
        assert_eq!(identify("0", 0), 1); // 0
        assert_eq!(identify("0~", 0), 1); // 0
        // Binary, near the end of `raw`.
        assert_eq!(identify("0b", 0), 0); // rejected, no binary value
        assert_eq!(identify("0B", 0), 1); // 0, "B" is not like "b"
        assert_eq!(identify("0b_", 0), 0); // rejected, no binary value
        assert_eq!(identify("0b2", 0), 0); // rejected, out of range
        assert_eq!(identify("0b12", 0), 0); // rejected, out of range
        assert_eq!(identify("0b_1", 0), 4); // 0b_1
        assert_eq!(identify("0b1_", 0), 4); // 0b1_
        assert_eq!(identify("0b1.", 0), 0); // binary float is not allowed
        assert_eq!(identify("0b1.1", 0), 0); // binary float is not allowed
        assert_eq!(identify("0b1e1", 0), 3); // 0b1
        // Decimal integer, near the end of `raw`.
        assert_eq!(identify("1", 0), 1); // 1
        assert_eq!(identify("+1", 0), 0); // leading "+" can’t start the lexeme
        assert_eq!(identify("-1", 0), 0); // leading "-" can’t start the lexeme
        assert_eq!(identify("1_", 0), 2); // 1_
        assert_eq!(identify("_1", 0), 0); // leading underscore is not allowed
        assert_eq!(identify("1_1", 0), 3); // 1_1
        assert_eq!(identify("1__1", 0), 4); // 1__1
        // Float, near the end of `raw`.
        assert_eq!(identify("1.", 0), 2); // 1.
        assert_eq!(identify("1.1", 0), 3); // 1.1
        assert_eq!(identify("1e", 0), 0); // 1
        assert_eq!(identify("1E", 0), 0); // 1
        assert_eq!(identify("1e1", 0), 3); // 1e1
        assert_eq!(identify("1E1", 0), 3); // 1E1
        assert_eq!(identify("1.e1", 0), 4); // 1 // @TODO fix this!
        assert_eq!(identify("1.E1", 0), 4); // 1 // @TODO fix this!
        assert_eq!(identify("1.1e", 0), 0); // rejected, no exponent value
        assert_eq!(identify("1.1E", 0), 0); // rejected, no exponent value
        assert_eq!(identify("1e+1", 0), 4); // 1e+1
        assert_eq!(identify("1E+1", 0), 4); // 1E+1
        assert_eq!(identify("1e-1", 0), 4); // 1e-1
        assert_eq!(identify("1E-1", 0), 4); // 1E-1
        assert_eq!(identify("1e+", 0), 0); // rejected, trailing sign after +
        assert_eq!(identify("1E+", 0), 0); // rejected, trailing sign after +
        assert_eq!(identify("1e-", 0), 0); // rejected, trailing sign after -
        assert_eq!(identify("1E-", 0), 0); // rejected, trailing sign after -
        // Hex, near the end of `raw`.
        assert_eq!(identify("0x", 0), 0); // rejected, no hex value
        assert_eq!(identify("0X", 0), 1); // 0, "X" is not like "x"
        assert_eq!(identify("0x_", 0), 0); // rejected, no hex value
        assert_eq!(identify("0xG", 0), 0); // rejected, out of range
        assert_eq!(identify("0x1g", 0), 3); // 0x1 @TODO maybe follow "0b12" behaviour?
        assert_eq!(identify("0x_1", 0), 4); // 0x_1
        assert_eq!(identify("0x1_", 0), 4); // 0x1_
        assert_eq!(identify("0x1.", 0), 0); // hex float is not allowed
        assert_eq!(identify("0x1.1", 0), 0); // hex float is not allowed
        assert_eq!(identify("0x1e", 0), 4); // 0x1e not enterpreted as exp
        assert_eq!(identify("0x1E", 0), 4); // 0x1E not enterpreted as exp
        assert_eq!(identify("0x1e1", 0), 5); // 0x1e1 not enterpreted as exp
        assert_eq!(identify("0x1E1", 0), 5); // 0x1E1 not enterpreted as exp
        assert_eq!(identify("0x1e+1", 0), 4); // 0x1e1 not enterpreted as exp
        assert_eq!(identify("0x1E+1", 0), 4); // 0x1E1 not enterpreted as exp
        assert_eq!(identify("0x1e-1", 0), 4); // 0x1e not enterpreted as exp
        assert_eq!(identify("0x1E-1", 0), 4); // 0x1E not enterpreted as exp
        assert_eq!(identify("0x1e+", 0), 4); // 0x1e not enterpreted as exp
        assert_eq!(identify("0x1E+", 0), 4); // 0x1E not enterpreted as exp
        assert_eq!(identify("0x1e-", 0), 4); // 0x1e not enterpreted as exp
        ...
        assert_eq!(identify("0o", 0), 0); // rejected, no hex value
        assert_eq!(identify("0O", 0), 1); // 0, "O" is not like "o"
        assert_eq!(identify("0o_", 0), 0); // rejected, no hex value
        assert_eq!(identify("0o8", 0), 0); // rejected, out of range
        assert_eq!(identify("0o18", 0), 3); // 0o1 @TODO maybe follow "0b12" behaviour?
        assert_eq!(identify("0o_1", 0), 4); // 0o_1
        assert_eq!(identify("0o1_", 0), 4); // 0o1_
        assert_eq!(identify("0o1.", 0), 0); // octal float is not allowed
        assert_eq!(identify("0o1.1", 0), 0); // octal float is not allowed
        assert_eq!(identify("0o1e1", 0), 3); // 0o1
        // Invalid `pos`.
        assert_eq!(identify("123", 2), 3); // 2 is before "3", so in range
        assert_eq!(identify("123", 3), 3); // 3 is after "3", so incorrect
        assert_eq!(identify("123", 4), 4); // 4 is out of range
        assert_eq!(identify("123", 100), 100); // 100 is way out of range
        // Non-ascii.
        assert_eq!(identify("€", 1), 1); // part way through the three eurobytes
        assert_eq!(identify("1€", 0), 1); // non-ascii after 1
        assert_eq!(identify("1.€", 0), 2); // non-ascii after 1.
        assert_eq!(identify("1_€'", 0), 2); // non-ascii after 1_
        assert_eq!(identify("1e€'", 0), 0); // non-ascii after 1e
        assert_eq!(identify("0€", 0), 1); // non-ascii after 0
        assert_eq!(identify("0b€", 0), 0); // non-ascii after 0b
        assert_eq!(identify("0b0€", 0), 3); // non-ascii after 0b0
        assert_eq!(identify("0x€", 0), 0); // non-ascii after 0b
        assert_eq!(identify("0x0€", 0), 3); // non-ascii after 0b0
        assert_eq!(identify("0o€", 0), 0); // non-ascii after 0o
        assert_eq!(identify("0o0€", 0), 3); // non-ascii after 0o0
    }
