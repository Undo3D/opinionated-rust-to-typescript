...
pub fn detect_character(raw: &str, pos: usize) -> usize {
    ...
    match get_aot(raw, pos+2) {
        ...
        "u" =>
            ...
            pos + detect_unicode_char_length(raw, pos, len),
        // ...anything else:
        _ =>
...
fn detect_unicode_char_length(raw: &str, pos: usize, len: usize) -> usize {
...
mod tests {
    ...
    fn detect_character_correct() {
        ...
        let raw = "abcde'f'ghi";
        assert_eq!(detect(raw, 4), 4); // e'f
        assert_eq!(detect(raw, 5), 8); // 'f' advance three places
        assert_eq!(detect(raw, 6), 6); // f'g
        assert_eq!(detect(raw, 7), 7); // 'gh
        ...
        assert_eq!(detect(raw, 0), 0); // ± is 2 bytes wide
        assert_eq!(detect(raw, 2), 6); // '±' advance four places
        assert_eq!(detect(raw, 6), 6); // ∆ is 3 bytes wide
        assert_eq!(detect(raw, 9), 14); // '∆' advance five places
        assert_eq!(detect(raw, 14), 14); // \u{10FFFF} is 4 bytes wide
        assert_eq!(detect(raw, 18), 24); // '\u{10FFFF}' advance five places
        // Simple backslash.
        let raw = " -'\\n'- ";
        assert_eq!(detect(raw, 1), 1); // -'\n
        assert_eq!(detect(raw, 2), 6); // '\n' advance four places
        assert_eq!(detect(raw, 3), 3); // \n'-
        ...
        // 7-bit '\x00'.
        let raw = "'\\x4A'";
        assert_eq!(detect(raw, 0), 6); // '\x4A' advance to end
        assert_eq!(detect(raw, 1), 1); // \x4A'
        assert_eq!(detect(raw, 5), 5); // '
        let raw = " - '\\x0f' - ";
        assert_eq!(detect(raw, 3), 9); // '\x0f' advance 6 places
        // Unicode '\u{0}'.
        ...
        let raw = "'\\u{30aF}'";
        assert_eq!(detect(raw, 0), 10); // '\u{30aF}' advance to end
        assert_eq!(detect(raw, 1), 1); // \u{30aF}'
        assert_eq!(detect(raw, 2), 2); // u{30aF}'
    }
    ...
    fn detect_character_incorrect() {
        ...
        assert_eq!(detect("'\\u{abcde", 0), 0); // raw too short
        assert_eq!(detect("'\\u{12i4}'", 0), 0); // not a hex digit
        assert_eq!(detect("'\\u{100abCd}'", 0), 0); // too long
        assert_eq!(detect("'\\u{1234}", 0), 0); // raw too short
