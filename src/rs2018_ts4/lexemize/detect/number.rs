...
pub fn detect_number(raw: &str, pos: usize) -> usize {
    ...
    if c != "0" { return detect_number_decimal(raw, pos, len) }
    ...
    match get_aot(raw, pos + 1) {
        ...
        "b" => detect_number_binary(raw, pos, len),
        "x" => detect_number_hex(raw, pos, len),
        "o" => detect_number_octal(raw, pos, len),
        // Otherwise, this is a decimal number which starts with a zero.
        _ => detect_number_decimal(raw, pos, len),
    }
}
...
fn detect_number_binary(raw: &str, pos: usize, len: usize) -> usize {
...
fn detect_number_decimal(raw: &str, pos: usize, len: usize) -> usize {
...
fn detect_number_hex(raw: &str, pos: usize, len: usize) -> usize {
...
fn detect_number_octal(raw: &str, pos: usize, len: usize) -> usize {
...
mod tests {
    ...
    fn detect_number_correct() {
        ...
        assert_eq!(detect(raw, 0), 4); // 0b01
        assert_eq!(detect(raw, 1), 1); // b01
        assert_eq!(detect(raw, 2), 4); // 01 is recognised as decimal
        assert_eq!(detect(raw, 5), 11); // 0b0_0_
        assert_eq!(detect(raw, 12), 15); // the 0b1 part is accepted
        assert_eq!(detect(raw, 17), 23); // 0b__1_
        // Decimal integer.
        let raw = "7 0 3";
        assert_eq!(detect(raw, 0), 1); // 7
        assert_eq!(detect(raw, 1), 1); // space
        assert_eq!(detect(raw, 2), 3); // 0
        assert_eq!(detect(raw, 3), 3); // space
        assert_eq!(detect(raw, 4), 5); // 3
        let raw = "765 012 10";
        assert_eq!(detect(raw, 0), 3); // 765
        assert_eq!(detect(raw, 1), 3); // 65 no ‘lookbehind’ happens!
        assert_eq!(detect(raw, 2), 3); // 5
        assert_eq!(detect(raw, 3), 3); // space
        assert_eq!(detect(raw, 4), 7); // 012
        assert_eq!(detect(raw, 7), 7); // space
        assert_eq!(detect(raw, 8), 10); // 10
        assert_eq!(detect(raw, 9), 10); // 0
        // Decimal with underscores.
        let raw = "7_5 012___ 3_4_. 0_0.0_00__0_";
        assert_eq!(detect(raw, 0), 3); // 7_5
        assert_eq!(detect(raw, 1), 1); // _5 can’t start numbers that way!
        assert_eq!(detect(raw, 2), 3); // 5
        assert_eq!(detect(raw, 4), 10); // 012___
        assert_eq!(detect(raw, 11), 16); // 3_4_.
        assert_eq!(detect(raw, 17), 29); // 0_0.0_00__0_
        // Float no exponent.
        let raw = "7.5 0.12 34. 00.0__0_00";
        assert_eq!(detect(raw, 0), 3); // 7.5
        assert_eq!(detect(raw, 1), 1); // .5 is not a valid number
        assert_eq!(detect(raw, 2), 3); // 5
        assert_eq!(detect(raw, 3), 3); // space
        assert_eq!(detect(raw, 4), 8); // 0.12
        assert_eq!(detect(raw, 9), 12); // 34. is valid
        assert_eq!(detect(raw, 13), 23); // 00.0__0_00
        // Here, each "123." exercises a different conditional branch.
        let raw = "123. 123.";
        assert_eq!(detect(raw, 0), 4); // 123. part way through input
        assert_eq!(detect(raw, 5), 9); // 123. reaches end of input
        // Float with exponent.
        let raw = "0e0 9E9 1e+2 4E-3 8E1+2 54.32E+10";
        assert_eq!(detect(raw, 0), 3);   // 0e0 is 0
        assert_eq!(detect(raw, 4), 7);   // 9E9 is 9000000000
        assert_eq!(detect(raw, 8), 12);  // 1e+2 is 100
        assert_eq!(detect(raw, 13), 17); // 4E-3 is 0.004
        assert_eq!(detect(raw, 18), 21); // the 8E1 part is accepted
        assert_eq!(detect(raw, 24), 33); // 54.32E+10 is 543200000000
        let raw = "4_3.21e+10 43_.21e+10 43.2_1e+10 43.21_e+10 43.21e+_10 43.21e+1_0 43.21e+10_";
        assert_eq!(detect(raw, 0), 10);  // 4_3.21e+10 is ok .js
        assert_eq!(detect(raw, 11), 21); // 43_.21e+10 is invalid in .js
        assert_eq!(detect(raw, 22), 32); // 43.2_1e+10 is ok .js
        assert_eq!(detect(raw, 33), 43); // 43.21_e+10 is invalid in .js
        assert_eq!(detect(raw, 44), 54); // 43.21e+_10 is invalid in .js
        assert_eq!(detect(raw, 55), 65); // 43.21e+1_0 is ok .js
        assert_eq!(detect(raw, 66), 76); // 43.21e+10_ is invalid in .js
        assert_eq!(detect("43.21e_10", 0), 9); // 43.21e_10 is invalid in .js
        // Hex.
        let raw = "0x09 0xA_b_ 0xAG 0x__C_";
        assert_eq!(detect(raw, 0), 4);   // 0x09
        assert_eq!(detect(raw, 1), 1);   // x09
        assert_eq!(detect(raw, 2), 4);   // 09 is recognised as decimal
        assert_eq!(detect(raw, 5), 11);  // 0xA_b_ mixed case is ok
        assert_eq!(detect(raw, 12), 15); // the 0xA part is accepted
        assert_eq!(detect(raw, 17), 23); // 0x__C_
        // Octal.
        let raw = "0o07 0o7_3_ 0o7a 0o__5_";
        assert_eq!(detect(raw, 0), 4);   // 0o07
        assert_eq!(detect(raw, 1), 1);   // o07
        assert_eq!(detect(raw, 2), 4);   // 07 is recognised as decimal
        assert_eq!(detect(raw, 5), 11);  // 0o7_3_
        assert_eq!(detect(raw, 12), 15); // the 0o7 part is accepted
        assert_eq!(detect(raw, 17), 23); // 0o__5_
    }
    ...
    fn detect_number_incorrect() {
        ...
        assert_eq!(detect(raw, 0), 0); // 0b12 is not a valid number
        assert_eq!(detect(raw, 2), 4); // 12 is recognised as decimal
        assert_eq!(detect(raw, 5), 5); // 0b11.1 is not a valid number
        assert_eq!(detect(raw, 7), 11); // 11.1
        assert_eq!(detect(raw, 12), 12); // 0b is not a valid number
        assert_eq!(detect(raw, 15), 16); // 0B11 is not valid, but 0 is
        assert_eq!(detect(raw, 20), 20); // 0b___ is not a valid number
        // Decimal integer.
        // @TODO
        // Incorrect float no exponent.
        let raw = "1.2.3 .12 0..1";
        assert_eq!(detect(raw, 0), 3); // 1.2
        assert_eq!(detect(raw, 1), 1); // .2 is not a valid number
        assert_eq!(detect(raw, 2), 5); // 2.3
        assert_eq!(detect(raw, 5), 5); // space
        assert_eq!(detect(raw, 6), 6); // .12 is not a valid number
        assert_eq!(detect(raw, 7), 9); // 12
        assert_eq!(detect(raw, 10), 12); // 0.
        assert_eq!(detect(raw, 11), 11); // ..
        assert_eq!(detect(raw, 12), 12); // .1
        assert_eq!(detect(raw, 13), 14); // 1
        // Incorrect float with exponent.
        let raw = "10e 9E+ 1e2. 4E+-3 8Ee12 1+1 54.32E";
        assert_eq!(detect(raw, 0), 0);   // 10e has no exponent value
        assert_eq!(detect(raw, 4), 4);   // 9E+ has no exponent value
        assert_eq!(detect(raw, 8), 8);   // 1e2. exponent value contains "."
        assert_eq!(detect(raw, 13), 13); // 4E+-3 has "+" and "-"
        assert_eq!(detect(raw, 19), 19); // 8Ee12 has an extra "e"
        assert_eq!(detect(raw, 21), 21); // e12 has no digit at start
        assert_eq!(detect(raw, 25), 26); // 1+1 perhaps you meant 1e+1
        assert_eq!(detect(raw, 29), 29); // 54.32E has no exponent value
        ...
        assert_eq!(detect(raw, 0), 0);   // 43._21e+10
        assert_eq!(detect(raw, 11), 11); // 43.21e_+10
        assert_eq!(detect(raw, 22), 22); // 43.21e_+
        assert_eq!(detect(raw, 31), 31); // 43.21e_ part way through input
        assert_eq!(detect(raw, 39), 39); // 43.21e_ reaches end of input
        // Invalid hex.
        let raw = "0xGA 0xab.c 0x 0XAB 0x___";
        assert_eq!(detect(raw, 0), 0);   // 0xGA is not a valid number
        assert_eq!(detect(raw, 5), 5);   // 0xab.c is not a valid number
        assert_eq!(detect(raw, 7), 7);   // ab.c is valid, but not a number
        assert_eq!(detect(raw, 12), 12); // 0x is not a valid number
        assert_eq!(detect(raw, 15), 16); // 0XAB is not valid, but 0 is
        assert_eq!(detect(raw, 20), 20); // 0x___ is not a valid number
        // Incorrect octal.
        let raw = "0oa7 0o56.7 0o 0O34 0o___";
        assert_eq!(detect(raw, 0), 0);   // 0oa7 is not a valid number
        assert_eq!(detect(raw, 5), 5);   // 0o56.7 is not a valid number
        assert_eq!(detect(raw, 7), 11);  // 56.7 is recognised as decimal
        assert_eq!(detect(raw, 12), 12); // 0o is not a valid number
        assert_eq!(detect(raw, 15), 16); // 0O34 is not valid, but 0 is
        assert_eq!(detect(raw, 20), 20); // 0o___ is not a valid number
        // Number too large.
    ...
    fn detect_number_will_not_panic() {
        ...
        // Decimal integer, near the end of `raw`.
