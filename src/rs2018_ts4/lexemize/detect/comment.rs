...
pub fn detect_comment(raw: &str, pos: usize) -> usize {
    ...
    match get_aot(raw, pos+1) {
        ...
        "/" => detect_inline_comment(raw, pos, len),
        // An asterisk, `pos` could begin a multiline comment.
        "*" => detect_multiline_comment(raw, pos, len),
...
fn detect_inline_comment(raw: &str, pos: usize, len: usize) -> usize {
...
fn detect_multiline_comment(raw: &str, pos: usize, len: usize) -> usize {
...
mod tests {
    ...
    fn detect_comment_inline() {
        // With newline.
        let raw = "abc//ok\nxyz";
        assert_eq!(detect(raw, 2), 2); // c//o
        assert_eq!(detect(raw, 3), 7); // //ok advance four places
        assert_eq!(detect(raw, 4), 4); // /ok<NL>
        // Without newline.
        let raw = "abc//okxyz";
        assert_eq!(detect(raw, 2), 2);  // c//o
        assert_eq!(detect(raw, 3), 10); // //okxyz advance to the end
        assert_eq!(detect(raw, 4), 4);  // /okxyz
        // With Windows line ending. The carriage return, '\r ', is treated like
        // any other character.
        let raw = "abc//ok\r\nxyz";
        assert_eq!(detect(raw, 2), 2); // c//ok
        assert_eq!(detect(raw, 3), 8); // //ok<CR> advance five places
        assert_eq!(detect(raw, 4), 4); // /ok<CR><NL>
        // Non-ascii.
    ...
    fn detect_comment_multiline_basic() {
        // Contains newline.
        let raw = "abc/*ok\n*/z";
        assert_eq!(detect(raw, 2), 2);  // c/*ok<NL>*
        assert_eq!(detect(raw, 3), 10); // /*ok<NL>*/ adv. seven places
        assert_eq!(detect(raw, 4), 4);  // *ok<NL>*/z
        // Doc.
        ...
        let raw = "abc/*ok*/";
        assert_eq!(detect(raw, 2), 2); // c/*ok*/
        assert_eq!(detect(raw, 3), 9); // /*ok*/ advance to the end
        assert_eq!(detect(raw, 4), 4); // *ok*/
        // Minimal.
        let raw = "//";
        assert_eq!(detect(raw, 0), 2);  // //
        assert_eq!(detect(raw, 1), 1);  // /
        let raw = "//\n";
        assert_eq!(detect(raw, 0), 3);  // //<NL>
        assert_eq!(detect(raw, 1), 1);  // /<NL>
        let raw = "/**/";
        assert_eq!(detect(raw, 0), 4);  // /**/
        assert_eq!(detect(raw, 1), 1);  // **/
        // Without end.
        let raw = "abc/*nope*";
        assert_eq!(detect(raw, 2), 2); // c/*nope*
        assert_eq!(detect(raw, 3), 3); // /*nope* malformed
        assert_eq!(detect(raw, 4), 4); // *nope*
    }
    ...
    fn detect_comment_multiline_nested() {
        ...
        assert_eq!(detect(raw, 0), 29); // does not end after ...inner */
        assert_eq!(detect(raw, 9), 20); // just catched /* inner */
        // Complex nesting.
        let raw = "pre-/* 0 /* 1 */ 0 /* 2 /* 3 */ 2 */ 0 */-post";
        assert_eq!(detect(raw, 3), 3);  // -/* 0
        assert_eq!(detect(raw, 4), 41); // /* 0 ... 0 */
        assert_eq!(detect(raw, 5), 5);  // * 0
        assert_eq!(detect(raw, 9), 16); // /* 1 */
        assert_eq!(detect(raw, 19), 36); // /* 2 /* 3 */ 2 */
        // `detect_comment()`’s loop deals with these edge cases correctly, by
        // stepping forward one extra pos after finding a nested "/*" or "*/".
        let raw = "/*/*/ */ */";
        assert_eq!(detect(raw, 0), 11); // /*/*/ */ */ edge case is the 3rd /
        assert_eq!(detect(raw, 1), 1);  // */*/ */ */
        assert_eq!(detect(raw, 2), 8);  // /*/ */
        let raw = "/*/* */* */";
        assert_eq!(detect(raw, 0), 11); // /*/* */* */ edge case is the 4th *
        assert_eq!(detect(raw, 1), 1);  // */* */* */
        assert_eq!(detect(raw, 2), 7);  // /* */
        // Invalid nesting.
        let raw = "/* outer /* inner */ missing trailing slash *";
        assert_eq!(detect(raw, 0), 0);
    }
